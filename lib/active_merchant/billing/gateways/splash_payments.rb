module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    class SplashPaymentsGateway < Gateway
      ...
      self.homepage_url = 'https://www.splashpayments.com/'
      self.display_name = 'Splash Payments'
      ...
      def capture(money, token, options = {})
        commit(:put, "txns/#{CGI.escape(token)}/capture", { amount: amount(money) }, options)
      end
      ...
      def add_invoice(post, options)
        post[:description] = options[:description] || "Trexle Core Purchase"
      end
      ...
      def unparsable_response(raw_response)
        message = "Invalid JSON response received from Splash Payments. Please contact support@splashpayments.com if you continue to receive this message."
